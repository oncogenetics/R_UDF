sampleLocation <- function(targetSamples,SampleLocRaw="Path:/to/progeny/DNA/output.txt" )
{
  SampleLocRaw <- read.delim(SampleLocRaw, na.strings=c("NA","","./."), as.is=T)
  
  #Filter ALL sample by target samples
  SampleLocRaw <- SampleLocRaw[ SampleLocRaw$Study.ID %in% targetSamples$Study.ID, ]
  ...
  #Test custom sort -------
  #http://stackoverflow.com/questions/1568511/how-do-i-sort-one-vector-based-on-values-of-another
  #testOrder <- c("Primary", "Secondary", "Stock")
  #testOrder[order(match(testOrder,SamOrder))]
  #[1] "Secondary" "Primary"   "Stock"  
  
  #set sample preference left -> right
  SamOrder <- c("Secondary", "Primary", "Stock", "Surplus", "Dilution")
  
  selectvec <- sapply(unique(SampleLoc$Study.ID), function(x) {
    ...
    unsorted[order(match(t(as.data.frame(SampleLoc[unsorted, "Plate.Type"], row.names=unsorted, stringsAsFactors=F)), SamOrder))][1]
  })
